...
class Socket extends Emitter {
    ...
    async bind(params) {
        if (this.isServer === false) {
            throw new Error('La socket può essere utilizzata in una sola modaliatà!');
        }
        this.isServer = true;
    ...
    connect(options) {
        ...
        this.client._socketId = this.socketId;
    ...
    initializeSocket(socket) {
        ...
        console.log('SOCKETID NAME', socket._socketId);
        ...
        socket.on('close', () => {
            ...
            console.log(`CLose su ${socket._socketId} isClient ${isClient} e ${!this.sockets[socket._socketId]} `);
        ...
        socket.on('timeout', () => {
            ...
            console.log('TIMEOUTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT');
            socket.destroy();
        });
        socket.on('error', (err) => {
            for (let i = 0; i < socket._pendingWritePromises.length; i++) {
                socket._pendingWritePromises[i].reject(err);
            }
            socket._pendingWritePromises = [];
            console.log('AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA', err);
        ...
        socket.on('drain', () => {
            ...
            socket._pendingWritePromises = [];
            this.emit(SocketEvents.bufferDrain, socket._socketId);
        });
    ...
    async _send(header, payload, system = false, socket) {
        if (!socket) throw new Error(`Socket ${this.socketId}: Tentativo di write su socket chiusa o non inizializzata`);
        ...
        console.log('SCRITTO SU SOCKET OK', socket._socketId, ok, socket.bufferSize, socket.writableHighWaterMark);
        payload = null;
        header = null;

        if (!ok && socket.bufferSize > this.config.maxBufferSize) {
            this.emit(SocketEvents.bufferFull, socket._socketId);
            let defer = new Deferred();
            socket._pendingWritePromises.push(defer);
            console.log('TORNATO TEST1', socket._socketId, ok);
            await defer.wait();

            console.log('TORNATO TEST2', socket._socketId, ok);
        }
    ...
    send(header, payload, socketId) {
        let socket;
        if (this.isServer && !socketId) throw new Error('Socket id obbligatoria');
        if (this.isServer) {
